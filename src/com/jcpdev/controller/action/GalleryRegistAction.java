...
public class GalleryRegistAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		String path = "D:\\C_WORK\\upload"; // 프로젝트 폴더와 관련 없는 경로일때 url 매핑을 server.xml에 설정한다.
		//프로젝트 폴더의 하위 경로일때 url 매핑을 server.xml에 설정한다.
		// String path = request.getServletContext().getRealPath("/image"); 
		//			ㄴ 업로드 폴더가 프로젝트 폴더의 하위 경로 image  폴더일때 사용합니다.

		int size = 10 * 1024 * 1024;	//10MByte, 최대파일크기
		try {
			
			//업로드된 파일을 받을 수 있는 MultipartRequest타입의 request객체 생성.
